...
export async function ensureDatabase(){
 ...
 ready=(async()=>{const sql=db();
  ...
  await sql`CREATE TABLE IF NOT EXISTS county_business_aliases (normalized_name TEXT PRIMARY KEY,business_name TEXT NOT NULL,economic_profile_id TEXT,linked_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),linked_by_employee TEXT)`;
  await sql`CREATE TABLE IF NOT EXISTS county_business_tax_ledger (
    id BIGSERIAL PRIMARY KEY,public_id TEXT NOT NULL UNIQUE DEFAULT ('CBT-'||EXTRACT(YEAR FROM NOW())::int::text||'-'||LPAD(nextval('county_tax_ledger_seq')::text,7,'0')),
    source TEXT NOT NULL,record_kind TEXT NOT NULL,business_name TEXT NOT NULL,normalized_business_name TEXT NOT NULL,player_name TEXT,
    category_code TEXT,tax_rate_basis_points INTEGER NOT NULL DEFAULT 0,gross_amount NUMERIC(14,2) NOT NULL DEFAULT 0,tax_amount NUMERIC(14,2) NOT NULL DEFAULT 0,payment_amount NUMERIC(14,2) NOT NULL DEFAULT 0,
    occurred_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),economic_profile_id TEXT,metadata JSONB NOT NULL DEFAULT '{}'::jsonb,created_at TIMESTAMPTZ NOT NULL DEFAULT NOW())`;
