...
export async function ensureDatabase(){
 ...
 ready=(async()=>{const sql=db();
  ...
  await sql`ALTER TABLE county_business_tax_ledger ADD COLUMN IF NOT EXISTS deduction_amount NUMERIC(14,2) NOT NULL DEFAULT 0`;
  await sql`ALTER TABLE county_business_tax_ledger ADD COLUMN IF NOT EXISTS filing_reference TEXT`;
  ...
  await sql`CREATE TABLE IF NOT EXISTS county_public_filing_attempts (
    id BIGSERIAL PRIMARY KEY,
    taxpayer_id TEXT,
  ...
  await sql`CREATE INDEX IF NOT EXISTS county_public_filing_attempts_lookup_idx
    ON county_public_filing_attempts(taxpayer_id,ip_hash,attempted_at DESC)`;
